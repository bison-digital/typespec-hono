...
import { tResponses, thingSchema } from "./schemas.gen.js";
...
export interface Operations {
	t(ctx: Ctx, input: z.infer<typeof thingSchema>): Awaitable<Result<z.infer<typeof thingSchema>>>;
}
 ...
 */
export type THandler = Operations["t"];
...
export function registerRoutes<T extends Operations>(
	...
	return app.post("/t", zValidator("json", thingSchema, deps.invalid), async (c) => {
		const ctx = deps.context(c, "none");
		if (ctx === null) return deps.noContext(c);
		return deps.respond(
			c,
			tResponses,
			await handlersFor(c).t(ctx, {
				...c.req.valid("json"),
			}),
		);
	});
