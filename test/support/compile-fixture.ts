...
export interface FixtureOptions {
	 ...
	 * produces.
	 *
	 * **This exists because every other compile in this suite used to configure the default away, and a
	 * defect lived in the gap for the whole life of the package.** `runtime-module` was set by this
	 * helper and by the corpus harness, so nothing ever compiled with the emitter's own default, which
	 * pointed at a module that exported none of the names `app.gen.ts` imports. The option is refused
	 * now and every compile uses the emitted runtime, but a compile carrying no options at all is still
	 * a different configuration from one carrying the contracts and sealing, and it is the one a new
