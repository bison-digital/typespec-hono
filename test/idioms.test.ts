 ...
 *
 * **Every rule below is Hono's own, from its best-practices guide, and not a house style.** That
...
describe("the emitted server follows Hono's own best practices", () => {
	...
	it("writes every handler inline, after the path it serves", () => {
		/**
		 * **Asserted as "no extracted handler exists", not as "some inline handler exists".** The
	...
	it("contains no type assertion, because a cast in generated code is nobody's to review", () => {
		/**
		 * **A cast in emitted output is worse than one in hand-written code.** Nobody reviews it, it
		 ...
		 *
		 * **The fix was to CHECK rather than to overload.** Overloading `context` so `"none"` cannot
		 ...
		 */
		/**
		 * **Comments are stripped first, and the first draft of this arm did not.** It matched the
